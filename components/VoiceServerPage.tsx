...
const VoiceServerPage: React.FC<{ currentUser: User }> = ({ currentUser }) => {
    ...
    const fetchRooms = useCallback(async () => {
        const res = await fetch(`/api?_ep=apps&app=voiceserver&type=rooms`, { headers: { 'x-user-id': currentUser.id } });
    ...
    const pollData = useCallback(async (roomId: string) => {
        const [pRes, mRes] = await Promise.all([
            fetch(`/api?_ep=apps&app=voiceserver&type=participants&roomId=${roomId}`, { headers: { 'x-user-id': currentUser.id } }),
            fetch(`/api?_ep=apps&app=voiceserver&type=messages&roomId=${roomId}&since=${lastMsgTs.current}`, { headers: { 'x-user-id': currentUser.id } })
        ]);
    ...
    const joinRoom = useCallback(async (room: VoiceServerRoom) => {
        ...
        await fetch('/api?_ep=apps&app=voiceserver', {
    ...
    const startRecording = async () => {
        ...
        mediaRecorderRef.current.ondataavailable = async (e) => {
            ...
            if (selectedRoom) {
                await fetch('/api?_ep=apps&app=voiceserver&type=message', {
                    method: 'POST', headers: { 'Content-Type': 'application/json', 'x-user-id': currentUser.id },
