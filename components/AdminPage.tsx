...
const AdminPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (currentUser) { // Edit user
      await fetch(`/api/users?id=${currentUser.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(formData) });
      if (password.trim()) {
        await fetch(`/api/users?id=${currentUser.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password }) });
      }
    } else { // Add user
      await fetch('/api/users', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userData: formData, password }) });
    }
    fetchUsers();
    closeModal();
  };
