...
const ContactsListPage: React.FC<{ currentUser: User }> = ({ currentUser }) => {
    ...
    const fetchContacts = useCallback(async () => {
        setIsLoading(true);
        try {
            const res = await fetch('/api?_ep=apps&app=contacts', { headers: { 'x-user-id': currentUser.id } });
    ...
    const handleSave = async () => {
        ...
        const url = `/api?_ep=apps&app=contacts` + (currentContact.id ? `&id=${currentContact.id}` : '');
        await fetch(url, {
            method,
    ...
    const handleDelete = async (contact: Contact) => {
        ...
        await fetch(`/api?_ep=apps&app=contacts&id=${contact.id}`, { 
