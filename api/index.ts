...
import type { User, CallStatus } from '../src/types';
...
async function handleGemini(req: VercelRequest, res: VercelResponse) {
    if (!process.env.API_KEY) return res.status(503).json({ message: 'Service Unavailable: AI service is not configured.' });
    ...
    const response = await ai.models.generateContent({
        ...
        contents: prompt,
        config: { systemInstruction: "You are Mason, a helpful assistant for LynxAI. Introduce yourself in the first message." }
    });
